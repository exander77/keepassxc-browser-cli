...
class Configuration {
    constructor(appName = "keepassxc-node-cli") {
    ...
    async load() {
        ...
        const configData = await fs_1.promises.readFile(this._configFile, {
            encoding: "utf-8"
        });
    ...
    async save() {
        ...
        await fs_1.promises.mkdir(path.dirname(this._configFile), { recursive: true });
        await fs_1.promises.writeFile(this._configFile, JSON.stringify(configData), {
            encoding: "utf-8"
        });
    }
    hasKey(dbHash) {
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.KeepassXCConnection = void 0;
const native_messaging_connector_1 = require("native-messaging-connector");
const nacl = require("tweetnacl");
const util = require("tweetnacl-util");
var Actions;
(function (Actions) {
    Actions["SET_LOGIN"] = "set-login";
    Actions["GET_LOGINS"] = "get-logins";
    Actions["GENERATE_PASSWORD"] = "generate-password";
    Actions["ASSOCIATE"] = "associate";
    Actions["TEST_ASSOCIATE"] = "test-associate";
    Actions["GET_DATABASE_HASH"] = "get-databasehash";
    Actions["CHANGE_PUBLIC_KEYS"] = "change-public-keys";
    Actions["LOCK_DATABASE"] = "lock-database";
    Actions["DATABASE_LOCKED"] = "database-locked";
    Actions["DATABASE_UNLOCKED"] = "database-unlocked";
    Actions["GET_DATABASE_GROUPS"] = "get-database-groups";
    Actions["CREATE_NEW_GROUP"] = "create-new-group";
})(Actions || (Actions = {}));
const KEY_SIZE = 24;
function getNonce() {
    return util.encodeBase64(nacl.randomBytes(KEY_SIZE));
}
function incrementedNonce(nonce) {
...
class KeepassXCConnection {
    ...
    static async create() {
        const connector = await native_messaging_connector_1.NativeConnector.create("org.keepassxc.keepassxc_browser");
    ...
    verifyKeyResponse(response, expectedNonce) {
    ...
    async sendMessage(action, timeout = 2000, message = null) {
        ...
        let response;
        do {
            response = await this._connection.readMessage(timeout);
    ...
    async changePublicKeys() {
        ...
        let response;
        do {
            response = await this._connection.readMessage(2000);
        ...
        return this.verifyKeyResponse(response, incNonce);
    }
    async getDatabaseHash() {
        if (!await this.verifyKeys()) {
            return null;
        }
        const response = await this.sendMessage(Actions.GET_DATABASE_HASH);
        if (!response.success) {
            return null;
        }
        this._version = response.version;
    ...
    async associate() {
        ...
        const response = await this.sendMessage(Actions.ASSOCIATE, null, {
    ...
    async testAssociate(id, idKey) {
        if (!await this.verifyKeys()) {
            return null;
        }
        const response = await this.sendMessage(Actions.TEST_ASSOCIATE, 2000, {
            id: id,
            key: idKey,
        });
        this._association = {
    ...
    async getLogins(url) {
        ...
        const response = await this.sendMessage(Actions.GET_LOGINS, 2000, {
            url: url,
            keys: [
                {
    ...
    static dispatchSignal(response) {
        console.log("Received signal: " + JSON.stringify(response));
    }
}
exports.KeepassXCConnection = KeepassXCConnection;

},{"native-messaging-connector":7,"tweetnacl":9,"tweetnacl-util":8}],3:[function(require,module,exports){

"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Inject = void 0;
const KeepassXCConnection_1 = require("./KeepassXCConnection");
const Configuration_1 = require("./Configuration");
async function associate(config, connection) {
    const dbHash = await connection.getDatabaseHash();
    if (!config.hasKey(dbHash) || !await connection.testAssociate(config.getKey(dbHash).id, config.getKey(dbHash).idKey)) {
...
async function Inject(login) {
    ...
    let username = null;
    let url = null;
    const preselect = process.env.processEnvs;
    if (preselect) {
    ...
    const config = new Configuration_1.Configuration("keepassxc-nativefier");
    await config.load();
    const connection = await KeepassXCConnection_1.KeepassXCConnection.create();
    try {
        ...
        let i = 0;
        for (let j = 0; i < logins.length; ++i) {
            if (username) {
                if (username === logins[i].login) {
                    if (userindex === j)
                        return login(logins[i]);
                    j++;
                }
            }
            else {
...
},{"./Configuration":1,"./KeepassXCConnection":2,"electron":"electron"}],4:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
window.onload = function () {
    ...
    function wrapped(c, t) {
        console.log('Next search', c, t);
        setTimeout(() => {
            if (logincheck()) {
                console.log('Running main...');
                inject_1.Inject(login).catch(err => {
                    alert(err);
...
},{"./inject":3}],5:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const os = require("os");
...
},{"os":undefined}],6:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const os = require("os");
const path = require("path");
...
const fs_1 = require("fs");
const NativeConnection_1 = require("./NativeConnection");
var ManifestSearchLocation;
(function (ManifestSearchLocation) {
    ManifestSearchLocation[ManifestSearchLocation["Firefox"] = 1] = "Firefox";
    ManifestSearchLocation[ManifestSearchLocation["Chrome"] = 2] = "Chrome";
    ManifestSearchLocation[ManifestSearchLocation["Chromium"] = 4] = "Chromium";
    ManifestSearchLocation[ManifestSearchLocation["Vivaldi"] = 8] = "Vivaldi";
    ManifestSearchLocation[ManifestSearchLocation["All"] = 63] = "All";
})(ManifestSearchLocation = exports.ManifestSearchLocation || (exports.ManifestSearchLocation = {}));
var NativeAppType;
(function (NativeAppType) {
    NativeAppType[NativeAppType["Firefox"] = 0] = "Firefox";
    NativeAppType[NativeAppType["Chrome"] = 1] = "Chrome";
    NativeAppType[NativeAppType["Chromium"] = 2] = "Chromium";
    NativeAppType[NativeAppType["Vivaldi"] = 3] = "Vivaldi";
})(NativeAppType = exports.NativeAppType || (exports.NativeAppType = {}));
function* getManifestLocations(search_location) {
...
async function findManifest(manifest_name, search_location) {
    for (const [searchPath, searchLocation] of getManifestLocations(search_location)) {
        ...
        try {
            const stats = await fs_1.promises.lstat(test_path);
            if (!stats.isFile()) {
                continue;
            }
...
class NativeConnector {
    constructor(manifest, app_type) {
        this._manifest = manifest;
        this._app_type = app_type;
    }
    connect() {
        const process = child_process.spawn(this._manifest.path);
        return new NativeConnection_1.NativeConnection(process, this._app_type);
    }
    static async create(manifest_name, search_location = ManifestSearchLocation.All) {
        ...
        const manifest_content = await fs_1.promises.readFile(manifest_path, { encoding: "utf-8" });
...
},{"./NativeConnection":5,"child_process":undefined,"fs":undefined,"os":undefined,"path":undefined}],7:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var NativeConnector_1 = require("./NativeConnector");
exports.NativeConnector = NativeConnector_1.NativeConnector;
exports.ManifestSearchLocation = NativeConnector_1.ManifestSearchLocation;
var NativeConnection_1 = require("./NativeConnection");
exports.NativeConnection = NativeConnection_1.NativeConnection;

},{"./NativeConnection":5,"./NativeConnector":6}],8:[function(require,module,exports){
